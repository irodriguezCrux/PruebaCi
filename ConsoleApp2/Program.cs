...
*/
            ...
            */
            ...
            Console.WriteLine(PalabrasEncontradasPractica("Conertertertsinpemovildigaleadiosalefectivo"));
        ...
        */
        ...
        public static void InvertirNumeroPractica(int numero) 
        {
            Console.Write(numero%10);
            if (numero>10)
            {
                InvertirNumeroPractica(numero / 10);
            }
        }

        public static int ObtenerConsonantes(string oracion) 
        {
            string consonantes = "BCDFGHJKLMÑPQRSTVWXYZ";
            string nuevaOracion = String.Empty;
            int cantidad = 0;
            ...
            {
                foreach (char item in oracion)
                {
                    if (consonantes.ToLower().Contains(item))
                    {
                        cantidad++;
                    }
                }
        ...
        public static string PalabrasLimpias(string oracion) 
        {
            string nuevaOracion = string.Empty;
        ...
        public static bool EncontrarLetraRepetida(string oracion) 
        {
            bool encontrado = false;
            char letraAnterior = char.MinValue;
            char[] lista = oracion.ToCharArray().OrderBy(x=>x).ToArray();

            if (!string.IsNullOrEmpty(oracion))
            {
                foreach (char item in lista)
                {
                    if (letraAnterior==item)
                    {
                        encontrado = true;
                    }
                    else
        ...
        public static void FibonacciPractica(int cantMax) 
        {
            int a = 0;
            int b = 1;
            int aux;
        ...
        public static int BinarioDecimalPractica(string binario) 
        {
            int valor = 0;
            int multi = 1;

            for (int posicion = binario.Length-1 ; posicion>=0 ; posicion--)
            {
                if (binario[posicion].ToString()!="0" )
                {
                    valor = multi + valor;
                }
                multi = multi * 2;
        ...
        public static string EsPalindromoPractica(string pal1, string pal2) 
        {
            ...
            if (pal1==nuevaPalabra)
            {
                mensaje = "Las palabras son Isomorfas";
            }
            else
            {
                mensaje = "Las palabras no son palindromas";
            }
        ...
        {
            char[] temp = palabra.Replace(" ", "").ToLower().ToCharArray();
            Array.Sort(temp);
            return new string(temp);

        }
        public static string EsAnagramaPractica(string palabra1, string palabra2) 
        {
                palabra1 = NuevoString(palabra1);
                palabra2 = NuevoString(palabra2);
                return palabra1.Equals(palabra2) ? "Son Anagramas" : "No son Anagramas";
        }

        public static void QuickSortPractica(ref int[] vector, int primero, int ultimo) 
        {
            int tmpLow = primero, tmpHi = ultimo, centro = (primero + ultimo) / 2;
            double pivot = vector[centro];

            do
            {
                while (vector[tmpLow] < pivot) tmpLow++ ;
                while (vector[tmpHi] > pivot) tmpHi-- ;

                if (tmpLow <= tmpHi)
                {
                    int temp = vector[tmpLow];
                    vector[tmpLow] = vector[tmpHi];
                    vector[tmpHi] = temp;
                    tmpLow++;tmpHi--;
                }
            ...
            if (primero<tmpHi){ QuickSortPractica(ref vector, primero, tmpHi); }
            if (tmpLow<ultimo){ QuickSortPractica(ref vector, tmpLow, ultimo); }
        }

        public static int SumaMaxPractica(int[] vector, int n) 
        {
            int sumatoria = 0;
        ...
        public static int SumaMaxMinPractica(int[] vector, int n) 
        {
            int sumatoria = 0;
            for (int i = 0; i <=n-1; i++)
            {
        ...
        public static List<string> PermutacionPractica(char [] lista) 
        {
            List<string> nuevaLista = new List<string>();
            nuevaLista.Add(lista[0].ToString());

            for (int i = 1; i < lista.Length; i++)
            {
                int currLen = nuevaLista.Count;
                ...
                for (int j = 0; j < currLen; j++)
                {
                    ...
                    for (int k = 0; k <=w.Length; k++)
                    {
                        string nstr = w.Insert(k, lista[i].ToString());
...
;                        }
        ...
        public static string PalabrasEncontradasPractica(string oracion) 
        {
            List<string> diccionario = new List<string>()
            {"Con", "sinpe", "movil", "digale", "adios", "al", "efectivo" }; 
            ...
            if (oracion!=null)
            {
                foreach (string item in diccionario)
                {
                    if (oracion.Contains(item))
                    {
                        nuevaPalabra += string.Concat(" ", item);
                    }
                }
            }
